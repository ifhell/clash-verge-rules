...
CONFIG = ROOT / "config" / "clash-verge.yaml"
...
# Region caps mirror the proxy groups in config/clash-verge.yaml, whose per-group
# filter is the single source for classification — keep group names below in sync
# with the region group names there.
REGION_CAP = int(os.getenv("REGION_CAP", "100"))
REGION_KEYS = ["港台", "东南亚", "日韩", "美加", "澳大利亚", "法国", "英国", "德国", "荷兰", "罗马尼亚", "西班牙"]


def load_region_filters() -> list[tuple[str, re.Pattern]]:
    groups = yaml.safe_load(CONFIG.read_text(encoding="utf-8")).get("proxy-groups", [])
    filters: list[tuple[str, re.Pattern]] = []
    seen: set[str] = set()
    for group in groups:
        name = str(group.get("name", ""))
        key = next((k for k in REGION_KEYS if k in name), None)
        if key and key not in seen and group.get("filter"):
            filters.append((key, re.compile(str(group["filter"]))))
            seen.add(key)
    missing = [k for k in REGION_KEYS if k not in seen]
    if missing:
        raise RuntimeError(f"region groups without filter in {CONFIG}: {', '.join(missing)}")
    return filters


REGION_FILTERS: list[tuple[str, re.Pattern]] = load_region_filters()
...
def fetch(url: str) -> dict:
    request = urllib.request.Request(url, headers={"User-Agent": "clash-verge-rules-aggregator/1.0"})
